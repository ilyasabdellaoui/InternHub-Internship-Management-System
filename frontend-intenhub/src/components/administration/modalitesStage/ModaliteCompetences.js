...
export default function ModaliteCompetences() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8080/competence/getAll");
          ...
          <div className="table-responsive">
            <table className="table mb-0" style={{width: '100%'}}>
              ...
              <tbody>
                <tr>
                  <td>
                    <select id="skillSelect" onChange={handleSkillSelectChange} className="form-select">
                    {competences &&
                      competences.length > 0 &&
                      competences.map((competence) => (
                        <option key={competence.codeCompetence} value={competence.codeCompetence}>
                          {competence.libelle}
                        </option>
                      ))}
